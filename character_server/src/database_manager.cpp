...
bool DatabaseManager::addCharacter(const CharacterData& character) {
    ...
    // Prepare statement
    std::string query = "INSERT INTO characters (name, surname, age, image, bio) VALUES (?, ?, ?, ?, ?)";
...
std::optional<CharacterData> DatabaseManager::getCharacter(int id) {
    ...
    std::string query = "SELECT id, name, surname, age, image, bio FROM characters WHERE id = ?";
    MYSQL_STMT* stmt = mysql_stmt_init(m_connection);
    if (!stmt) {
        return std::nullopt;
    }
    ...
    // Image
    result_bind[4].buffer_type = MYSQL_TYPE_BLOB;
    result_bind[4].buffer = image_data;
    result_bind[4].buffer_length = sizeof(image_data);
    result_bind[4].length = &image_length;
    ...
    result_bind[5].buffer = bio_buffer;
    result_bind[5].buffer_length = sizeof(bio_buffer);
    ...
    if (!is_null[4] && image_length > 0) {
        character.image.assign(image_data, image_data + image_length);
    }
